...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/user/check/?param="+$("#regName").val()+"&&type=1&&r=" + Math.random(),
	            	success : function(data) {
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl +  "/user/check/?param="+$("#phone").val()+"&&type=2&&r=" + Math.random(),
				            	success : function(data) {
				            		if (data.data) {
					            		
				            			$.ajax({
				            				url : REGISTER.param.surl + "/user/check/?param="+$("#email").val()+"&&type=3&&r=" + Math.random(),
							            	success : function(data) {
							            		if (data.data) {
								            		REGISTER.doSubmit();
							            		} else {
